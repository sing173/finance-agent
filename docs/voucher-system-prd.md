...
route(file_path)
  │
  ├── Level 0: 扩展名分流（现有，保留）
  │   .csv  → 直接路由（当前仅 ICBCCSVParser）
  │   .xlsx → 直接路由（当前仅 CMBExcelParser）
  │   .pdf  → fitz 检测是否有嵌入式文本 → 分支
  │
  ├── Level 1: 嵌入式 PDF → 结构特征匹配
  │   fitz 提取前 3 页文本 → _match_pdf_structure() 匹配标题+列头（all/any 模式）
  │   命中 → 返回 (bankCode, docType) → 查 PARSER_REGISTRY → 路由到对应 parser
  │   未命中 → 返回 ('未知银行', '流水') → 强制验证拦截 → 前端 ManualOverrideModal
  │
  └── Level 2: 扫描件/图片 PDF → OCR 账号匹配
      fitz 文本为空 → 渲染首页为图片 → RapidOCR（单例复用）
      → 正则提取账号 → account_registry.match_by_account()
      命中 → 返回 (bankCode, '回单') → 路由到对应 parser
      未命中 → 返回 ('未知银行', '回单') → 强制验证拦截 → 前端 ManualOverrideModal

强制验证：bankCode ∉ PARSER_REGISTRY → 拒绝解析（前后端双重护栏）
```
...
| 文件类型 | 主路径 | 兜底 |
|---------|--------|------|
| 嵌入式 PDF（招行、广发） | Level 1 结构特征匹配，命中即停 | 未命中 → 默认返回“未知银行 + 流水” |
| 扫描件/图片 PDF（工行回单） | Level 2 OCR → 账号匹配 | 未命中 → 默认返回“未知银行 + 回单” |

**强制验证规则**（防御性）：

无论哪种检测路径，`_do_parse_pdf()` 都会检查 `bankCode ∈ PARSER_REGISTRY`。不在注册表中的银行直接**拒绝解析**（返回 error），不会尝试任何 parser。前端同步验证：`useBatchOrchestrator.parseOnly` 在解析前检查 `bank` 和 `docType`，未配置则跳过该文件并提示手动选择。这套双重护栏确保：宁可让用户在 `ManualOverrideModal` 中手动选择银行，也不让系统在不确定的情况下静默解析错误。

#### Level 1：嵌入式 PDF 结构特征匹配

`fitz` 提取前 3 页文本，检测表标题+列头组合，匹配前先压缩空白以消除 PDF 渲染字间空格：
...
匹配规则：
- `mode='all'` — 压缩空白后所有关键字均须出现（表格格式需要标题+列头组合，如“账务明细清单”+“借方/贷方金额”）
- `mode='any'` — 任一关键字命中即触发（回单类只有标题无列头，如“出账回单”或“入账回单”）
- **Level 1 命中即停**，不进入 Level 2 OCR。后续可扩展为让客户自行配置结构特征规则
- 未命中（无匹配的结构特征）→ 返回 `('未知银行', '流水')`，由强制验证规则拦截解析
...
```
1. fitz 渲染首页为图片 → RapidOCR 识别文字（RapidOCR 单例复用，避免重复加载 ~50MB ONNX 模型）
2. 正则 \d{12,19} 提取所有候选账号
3. account_registry.match_by_account() —— 遍历候选账号，首个命中即返回
4. 命中 → 返回 (entry.bankCode, '回单')
5. 未命中 → 返回 ('未知银行', '回单')
```

`detect_bank_from_pdf()` 保留但重写：改为调用 `_match_pdf_structure(text)` 替代 `_classify()`。Level 1 和 Level 2 共享同一个 `fitz.Document` 实例（`try/finally` 确保关闭），避免重复读 PDF。

#### PARSER_REGISTRY：统一 bankCode → parser 映射

所有检测路径统一返回 `(bankCode, docType)`。parser 按注册表顺序逐个尝试，首个返回非空数据即停（不再有 fallback/trial_chain/DEFAULT_PARSER）：

```python
# bankCode 是路由 key，bank 中文名仅用于 UI 展示
# 每个 bank 条目是一个有序 list，依次尝试，首个返回 result.transactions 的即停
# dict 条目表示 sub-typed dispatch（如 CMB statement 的 table vs column 格式）
PARSER_REGISTRY: dict[str, list] = {
    'ICBC': [
        ('icbc_receipt_grid_parser', 'ICBCReceiptGridParser'),  # 回单优先
        ('icbc_parser', 'ICBCParser'),                            # 流水兜底
    ],
    'CMB': [
        ('cmb_receipt_parser', 'CMBReceiptParser'),              # 回单
        {'table': ('cmb_table_parser', 'CMBTableParser'),         # 流水 — 按 subtype 分发
         'column': ('cmb_parser', 'CMBParser')},
    ],
    'GFB': [
        ('gfb_table_parser', 'GFBTableParser'),                   # 仅流水
    ],
}
```

**`_do_parse_pdf()` bank 参数行为**：
- `bank` 非空 → **外部覆盖**，`BANK_NAME_TO_CODE.get(bank, bank)` 转为 bankCode，跳过 Level 1/2 检测，直接查 PARSER_REGISTRY
- `bank` 为空/None → 走完整三级路由检测（`detect_bank_from_pdf()`）

**强制验证**：`bank_code not in PARSER_REGISTRY` → 返回 `{ success: false, error: "无法识别银行类型..." }`（拒绝解析）。`_dispatch_registry_parser` 遍历 registry list 后仍无结果 → 同样拒绝。不做任何“静默兜底”。
...
- `detect_bank_from_pdf()` → 保留函数签名，重写为三级路由调度：L1 结构特征 → L2 账号匹配 → 兜底返回未知银行
- 新增 `_detect_bank_by_ocr_account()` — OCR 首页 + 正则提取账号 → account_registry.match_by_account()。复用传入的 `fitz.Document` 避免重复读 PDF
- `_do_parse_pdf()` 中的 `if '工商'/'招商'/'广发' in bank` 门控 → 删除，改为查 `PARSER_REGISTRY` 映射表
- `_dispatch_registry_parser()` — 遍历 registry list，首个返回非空数据即停；`dict` 条目按 CMB PDF subtype（table/column）分发
- `try_receipt_first` → 删除（ICBC registry 中 receipt grid parser 排在 statement parser 之前）
- `detect_cmb_pdf_type()` → 私有化为 `_detect_cmb_pdf_subtype()`
- `DEFAULT_PARSER`、`EMBEDDED_PDF_TRIAL_CHAIN`、`_run_trial_chain()` → 全部删除（改为强制拒绝而非静默兜底）
- RapidOCR 实例 → 模块级单例 `_get_ocr()`，避免重复加载 ONNX 模型
- 所有内部 docType 统一为中文：`'流水'` / `'回单'`

#### 前端变化

```
选择文件 → detectBanks([filePath])
  → 后端检测银行（三级路由：结构特征 / OCR 账号匹配）
...
`detect_banks` JSON-RPC 签名不变。前端改动：

- `shared/types.ts`：新增 `DocType = '流水' | '回单' | 'unknown'` 类型别名；`DetectFileResult` 增加 `bankCode?: string`；所有接口 docType 字段类型化为 `DocType`
- `ManualOverrideModal.tsx`：默认 docType 改为 `'流水'`（用户手动覆盖时预填）
- `useBatchOrchestrator.ts`：`parseOnly()` 新增强制验证 —— 银行未知或 docType 缺失的跳过解析，提示用户手动选择
- `App.tsx` 检测流程不变

唯一变化：检测结果更可靠（结构特征匹配比关键字二义性低，账号匹配能精确定位银行和科目）；未识别文件必须用户手动配置后才能解析。

#### 涉及改动的文件

| 文件 | 改动类型 | 说明 |
|------|---------|------|
| `parser_router.py` | **重构** | 新增 `PARSER_REGISTRY`（`dict[str, list]`）、`PDF_STRUCTURE_MATCHERS`（三元组格式含 `all`/`any` 模式）；`_classify()`→`_match_pdf_structure()`；新增 `_detect_bank_by_ocr_account()`（fitz doc 复用 + RapidOCR 单例）；`detect_bank_from_pdf()` 重写为三级路由调度；`_do_parse_pdf()` 门控改为 `PARSER_REGISTRY[bankCode]` 查表 + 强制验证；删除 `try_receipt_first`、`detect_cmb_pdf_type()`、`EMBEDDED_PDF_TRIAL_CHAIN`、`DEFAULT_PARSER`、`_run_trial_chain()` |
| `bridge.py` | **微调** | 删除 `_detect_bank_from_pdf()` 代理和 `detect_cmb_pdf_type()` 代理；`handle_detect_banks` 内联调用新路由函数 |
| `models.py` | **微调** | 新增 `AccountEntry` 数据类 |
| `account_registry.py` | **新增** | `match_by_account()` + CRUD 方法（list/add/update/delete），纯 JSON 文件读写 |
| `account_mapping.json` | **升级** | 直接采用 v2 格式（`id`/`matchType`/`pattern`/`bank`/`bankCode`/`subjectCode`/`subjectName`），保留 `defaultBankSubjectCode` |
| `shared/types.ts` | **微调** | 新增 `DocType` 类型别名；`DetectFileResult` 增加 `bankCode`；所有 docType 字段类型化 |
| `ManualOverrideModal.tsx` | **微调** | 默认 docType 改为 `'流水'` |
| `useBatchOrchestrator.ts` | **微调** | `parseOnly()` 新增强制验证（银行/docType 缺失拒绝解析） |
| `parser`（11 个） | **不变** | 照常解析，不受路由重构影响 |
| `App.tsx` | **不变** | 检测→解析→展示流程不变 |
...
| `pattern` | 匹配用的账号片段 |
| `bank` | 银行名称（选择 bankCode 后自动填充，**不可手动修改** — disabled Input） |
| `bankCode` | 银行简称（ICBC/CMB/GFB）——下拉选择，选项硬编码 `BANK_CODE_MAP` 常量；FR-0 PARSER_REGISTRY 路由 key |
...
| 新增 | 表单：匹配类型下拉（suffix/exact）、匹配模式输入框、银行代码下拉（选项硬编码 `BANK_CODE_MAP` 常量，选择后 `bank` 自动填充中文名且不可手动修改）、科目选择器（复用 `SubjectPickerModal`）。`bankCode` + `subjectCode` 均必填，`subjectCode` 须存在于 `subjects.json` 中 |
| 编辑 | 同新增表单，预填当前值 |
| 删除 | 确认后删除，不可恢复 |
...
| 文件 | 改动 |
|------|------|
| `parser_router.py` | **路由重构**：新增 `PARSER_REGISTRY`（`dict[str, list]`）、`PDF_STRUCTURE_MATCHERS`（`all`/`any` 二元组）；`_classify()`→`_match_pdf_structure()`；新增 `_detect_bank_by_ocr_account()`（fitz doc 复用 + RapidOCR 单例）；`_do_parse_pdf()` 门控改为 `PARSER_REGISTRY[bankCode]` 查表 + 强制验证；删除 `try_receipt_first`、`detect_cmb_pdf_type()`、`EMBEDDED_PDF_TRIAL_CHAIN`、`DEFAULT_PARSER`、`_run_trial_chain()` |
| `bridge.py` | 删除 `_detect_bank_from_pdf()` 代理和 `detect_cmb_pdf_type()` 代理；`handle_detect_banks` 内联调用新路由函数；docType 统一中文 |
...
| 文件 | 改动 |
|------|------|
| `bridge.py` | 新增 `account_registry.list`/`match`/`add`/`update`/`delete` JSON-RPC 方法（含 `get_subjects_info` 扩展返回 `subjects[]` 列表） |
| `apps/electron/src/ipc.ts` | 新增 5 个 `account_registry.*` channel |
| `apps/electron/src/preload.ts` | 新增 `invoke` 通用 JSON-RPC 方法 |
| `apps/renderer/src/App.tsx` | 新增"账号管理"入口卡片 |
| `AccountSubjectManager.tsx` | 新建 — Modal 表格 CRUD，bankCode 下拉联动 bank |
| `SubjectPickerModal.tsx` | 新建 — 科目搜索/分类筛选弹窗 |
...
| 文件 | 改动 |
|------|------|
| `apps/electron/src/ipc.ts` | 新增 5 个 `account_registry.*` channel 注册 |
| `apps/electron/src/preload.ts` | 新增 `invoke` 通用方法（供 AccountSubjectManager 调用 JSON-RPC） |
| `apps/renderer/src/App.tsx` | 新增"账号管理"入口卡片 |
...
| 方法 | 用途 |
|------|------|
| `detect_banks` | 批量检测文件银行类型（三级路由：Level 0 扩展名→Level 1 结构特征→Level 2 OCR 账号匹配）。`DetectFileResult` 增加 `bankCode` 字段；docType 统一中文（`'流水'` / `'回单'`） |
| `detect_supported_banks` | 查询支持银行列表（从 `PARSER_REGISTRY` keys → `BANK_CODE_TO_NAME` 动态返回中文名） |
| `parse_pdf` | 解析入口不变，后端路由改为 `PARSER_REGISTRY[bankCode]` 查表 + 强制验证（不在注册表中拒绝解析） |
...
| `account_registry.match` | 给定账号，返回匹配的科目+银行（Phase 0 已实现 match_by_account()，Phase 1 暴露为 JSON-RPC） |
...
| D14 | `bankCode` 输入方式 | **下拉选择**，选项硬编码 `BANK_CODE_MAP` 常量（与后端 `BANK_CODE_TO_NAME` 保持同步） | 新增银行需同时更新前端 `BANK_CODE_MAP` + 后端 `PARSER_REGISTRY` + `BANK_CODE_TO_NAME`，由程序控制可解析银行范围 |
...
| D19 | `bank` 字段 | **保留冗余存储**，选择 bankCode 后自动填充中文名，**不可手动修改**（disabled Input） | bankCode→bank 是确定性映射，禁用手动修改防止不一致 |
